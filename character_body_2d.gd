...
func _physics_process(delta: float) -> void:
	...
	if not ((direction/abs(direction) if direction else 0) == velocity.x/abs(velocity.x) or animation_player.current_animation == "idle" or animation_player.current_animation == "stop_running"):
